...
#### 3. 二軸電動舵機雲台（ZL-KPZ 控制板）
- **型號**: ZL-KPZ 控制板（AR版）
- **功能**: 控制總線舵機雲台，支援多個傳感器接口
- **連接方式**: USB 串口與 Orange Pi 通訊
- **傳感器接口**: 4 組（G/V/信號），可連接傳感器或雷射頭
  - 第 1 組傳感器接口：用於連接雷射頭
  - 第 2-4 組傳感器接口：可擴充其他傳感器
- **主控芯片**: ATmega328P（相容 Arduino Nano）
...
- **型號**: 5V 雷射頭模組（1mW 紅光）
- **功能**: 蚊子標定、射擊
- **連接**: 接在 ZL-KPZ 控制板的**第 1 組傳感器接口**
  - G：GND
  - V：5V (不連接)
  - S：由控制板數位輸出控制（Pin D6）→ 雷射頭 VCC
- **控制原理**: D6 輸出 HIGH (5V) 時雷射開啟，LOW (0V) 時雷射關閉
- **控制**: 由 Arduino 固件控制開關，無需額外 MOSFET
...
ZL-KPZ 控制板是一款基於 ATmega328P 的舵機控制板，相容 Arduino Nano 開發環境。
...
控制板提供 **6 組傳感器接口** (DJ0-DJ5)，每組 3 個引腳：

```
   DJ0       DJ1       DJ2       DJ3       DJ4       DJ5
(傳感器1)  (傳感器2)  (傳感器3)  (傳感器4)  (傳感器5)  (傳感器6)
...
**Arduino 引腳對映 (根據 ZL-KPZ 控制板定義)**:
| 傳感器接口 | 控制板標記 | Arduino Pin | 目前用途 |
|----------|----------|-------------|----------|
| 傳感器 1 | DJ0 | D7 | 可用 |
| 傳感器 2 | DJ1 | D3 | 可用 |
| 傳感器 3 | DJ2 | D5 | 可用 |
...
**連接說明**:
1. **G (GND)**: 連接雷射頭 GND
2. **V (5V)**: 不連接（雷射頭由信號引腳控制）
3. **S (Pin D6)**: 連接雷射頭 VCC，透過程式控制 HIGH/LOW 來開關雷射

**工作原理**:
- `digitalWrite(LASER_PIN, HIGH)` → Pin D6 輸出 5V → 雷射開啟
- `digitalWrite(LASER_PIN, LOW)` → Pin D6 輸出 0V → 雷射關閉

**注意**: 傳感器接口 DJ0-DJ5 (D7, D3, D5, D6, D9, D8) 都保留給傳感器使用，目前僅 DJ3 (D6) 用於雷射控制

**工作原理**:
- Arduino Pin D6 輸出 HIGH (5V) → 雷射開啟
- Arduino Pin D6 輸出 LOW (0V) → 雷射關閉
...
設備路徑: `/dev/ttyUSB0` 或 `/dev/ttyACM0`
