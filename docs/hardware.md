...
| **攝像頭** | 單一雙目 USB 1080p (3840×1080) | 1 | 基線 12cm，60fps，影像左右並排 |
...
**單一雙目 USB 攝像頭**（3840×1080 @ 60fps MJPEG）:
- **總解析度**: 3840×1080 (左右影像並排)
- **單眼解析度**: 1920×1080 @ 60fps (系統自動將影像分割為左右兩部分)
- **基線距離**: 12cm (深度估計用)
- **焦距**: 3.0mm
- **視角**: 96°（對角）
- **最低照度**: 0.5 lux
...
```
Orange Pi 5 / RDK X5
├─ USB 3.0 Port 1 ──→ 單一雙目攝像頭 (/dev/video0) - 系統自動分割左右影像
