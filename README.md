...
- 📺 **即時影像串流** (v2.4.0 新增):
  ...
  - Web 介面顯示即時統計（FPS、唯一目標數、連線數）
  - 多客戶端同時觀看支援
- � **光照度智能監測** (v2.5.0 新增):
  - 實時估算畫面光照強度（0-255 亮度值）
  - 光照過低時自動暫停 AI 檢測，減少誤報和計算浪費
  - 可配置光照閾值：警告（60）、暫停（40）
  - UI 底部右方顯示光照狀態：綠色（正常）、橙色（警告）、紅色（暫停）
  - 自動檢測間隔可調（預設 5 秒）
...
**Web 介面顯示:**
- 即時影像（包含 AI 檢測框、信心度、追蹤 ID）
- FPS、唯一目標數、追蹤狀態
- 連線客戶端數量

**系統狀態輸出:**
- 每 100 幀輸出一次統計信息（幀數、FPS、唯一目標數、追蹤狀態）
- 終端機顯示系統運行狀態
