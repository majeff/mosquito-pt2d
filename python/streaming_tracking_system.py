...
class StreamingTrackingSystem:
    ...
    def __init__(self,
        ...
        logger.info("\n📋 系統配置參數:")
        logger.info("\n[AI_DETECTION]")
        logger.info(f"  imgsz: {config.imgsz}")
        logger.info(f"  confidence_threshold: {config.confidence_threshold}")
        logger.info(f"  iou_threshold: {config.iou_threshold}")
        logger.info(f"  detection_mode: {config.detection_mode}")
        logger.info(f"  tile_overlap: {config.tile_overlap}")
        logger.info(f"  detection_margin: {config.detection_margin}")
        logger.info(f"  min_mosquito_size_mm: {config.min_mosquito_size_mm}")
        logger.info(f"  max_mosquito_size_mm: {config.max_mosquito_size_mm}")
        logger.info(f"  enable_white_pixel_filter: {config.enable_white_pixel_filter}")
        logger.info(f"  white_pixel_threshold: {config.white_pixel_threshold}")
        logger.info(f"  white_pixel_ratio_threshold: {config.white_pixel_ratio_threshold}")

        logger.info("\n[SINGLE_CAMERA_FILTER]")
        logger.info(f"  enable_bbox_size_filter: {config.enable_bbox_size_filter}")
        logger.info(f"  min_bbox_size_px: {config.min_bbox_size_px}")
        logger.info(f"  max_bbox_size_px: {config.max_bbox_size_px}")
        logger.info(f"  enable_aspect_ratio_filter: {config.enable_aspect_ratio_filter}")
        logger.info(f"  min_aspect_ratio: {config.min_aspect_ratio}")
        logger.info(f"  max_aspect_ratio: {config.max_aspect_ratio}")
        logger.info(f"  enable_temporal_filter: {config.enable_temporal_filter}")
        logger.info(f"  min_consecutive_frames: {config.min_consecutive_frames}")
        logger.info(f"  enable_motion_filter: {config.enable_motion_filter}")
        logger.info(f"  max_movement_px_per_frame: {config.max_movement_px_per_frame}")
        logger.info(f"  max_static_frames: {config.max_static_frames}")
        logger.info(f"  static_threshold_px: {config.static_threshold_px}")

        logger.info("\n[TRACKING]")
        logger.info(f"  pan_gain: {config.pan_gain}")
        logger.info(f"  tilt_gain: {config.tilt_gain}")
        logger.info(f"  no_detection_timeout: {config.no_detection_timeout}")
        logger.info(f"  target_lock_distance: {config.target_lock_distance}")

        logger.info("\n[HARDWARE]")
        logger.info(f"  arduino_port: {config.arduino_port}")
        logger.info(f"  left_camera_id: {config.left_camera_id}")
        logger.info(f"  pan_servo_id: {config.pan_servo_id}")
        logger.info(f"  tilt_servo_id: {config.tilt_servo_id}")

        logger.info("\n[ALERTS]")
        logger.info(f"  beep_cooldown: {config.beep_cooldown}")
        logger.info(f"  laser_cooldown: {config.laser_cooldown}")

        logger.info("\n[SAMPLE_COLLECTION]")
        logger.info(f"  save_high_confidence_samples: {config.save_high_confidence_samples}")
        logger.info(f"  save_uncertain_samples: {config.save_uncertain_samples}")
        logger.info(f"  uncertain_conf_range_min: {config.uncertain_conf_range[0]}")
        logger.info(f"  uncertain_conf_range_max: {config.uncertain_conf_range[1]}")
        logger.info(f"  max_samples: {config.max_samples}")
        logger.info(f"  save_interval: {config.save_interval}")
        ...
        logger.info("\n[STREAMING_SERVER]")
        logger.info(f"  stream_port: {config.stream_port}")
        logger.info(f"  stream_quality: {config.stream_quality}")
        logger.info(f"  stream_fps: {config.stream_fps}")
        logger.info(f"  rtsp_url: {config.rtsp_url}")
        logger.info(f"  rtsp_bitrate: {config.rtsp_bitrate}")
        logger.info(f"  rtsp_preset: {config.rtsp_preset}")

        logger.info("\n[TEMPERATURE_MONITORING]")
        logger.info(f"  enable_temperature_monitoring: {config.enable_temperature_monitoring}")
        logger.info(f"  temperature_warning_threshold: {config.temperature_warning_threshold}")
        logger.info(f"  temperature_pause_threshold: {config.temperature_pause_threshold}")
        logger.info(f"  temperature_resume_threshold: {config.temperature_resume_threshold}")
        logger.info(f"  temperature_check_interval: {config.temperature_check_interval}")
        logger.info(f"  temperature_sensor_path: {config.temperature_sensor_path}")

        logger.info("\n[ILLUMINATION_MONITORING]")
        logger.info(f"  enable_illumination_monitoring: {config.enable_illumination_monitoring}")
        logger.info(f"  illumination_warning_threshold: {config.illumination_warning_threshold}")
        logger.info(f"  illumination_pause_threshold: {config.illumination_pause_threshold}")
        logger.info(f"  illumination_check_interval: {config.illumination_check_interval}")

        logger.info("\n[CAMERA]")
        logger.info(f"  camera_dual_width: {config.camera_dual_width}")
        logger.info(f"  camera_dual_height: {config.camera_dual_height}")
        logger.info(f"  camera_dual_fps: {config.camera_dual_fps}")
        logger.info(f"  frame_delay: {config.frame_delay}")

        logger.info("\n[SERIAL]")
        logger.info(f"  arduino_baudrate: {config.arduino_baudrate}")
        logger.info(f"  arduino_timeout: {config.arduino_timeout}")

        logger.info("\n[DEPTH_ESTIMATION]")
        logger.info(f"  depth_focal_length: {config.depth_focal_length}")
        logger.info(f"  depth_baseline: {config.depth_baseline}")
        logger.info(f"  depth_sensor_width: {config.depth_sensor_width}")

        logger.info("\n[TRACKER]")
        logger.info(f"  position_update_interval: {config.position_update_interval}")
        logger.info("=" * 60 + "\n")
